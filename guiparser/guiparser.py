...
def get_chatbox_box(player_name):
    guiname = get_player_guiname(player_name)
    parser = GUIParser(guiname, {"ChatPanel_1": (0, 0)})
    coordinates = parser.get_box_coordinates("ChatPanel_1")
    x, y, _, _ = coordinates
    width = float(parser.get_element_object("ChatPanel_1").find("width").get("Value"))
    height = float(parser.get_element_object("ChatPanel_1").find("height").get("Value"))
...
class GUIParser(object):
    ...
    def get_item_value(element, name):
        ...
        try:
            ...
            raise ValueError("Element {0} did not have the expected attribute Value".format(name))
    ...
    def get_essential_element_values(self, element):
        ...
        try:
            ...
            x_offset = self.get_item_value(element, "anchorOffsetX")
            y_offset = self.get_item_value(element, "anchorOffsetY")
